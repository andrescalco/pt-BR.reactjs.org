...
Nós não anunciamos as [releases](https://github.com/facebook/react/blob/master/CHANGELOG.md#1530-july-29-2016) [anteriores](https://github.com/facebook/react/blob/master/CHANGELOG.md#1510-may-20-2016) [menores](https://github.com/facebook/react/blob/master/CHANGELOG.md#1520-july-1-2016)  no blog pois a maioria das mudanças eram correções de bug. No entando, 15.4.0 é um lançamento especial e nós gostaríamos de destacar algumas mudanças notáveis.

### Separando React e React DOM {#separating-react-and-react-dom}

[Há mais de um ano atrás](/blog/2015/09/10/react-v0.14-rc1.html#two-packages-react-and-react-dom), nós começamos a separar React and React DOM em pacotes distintos. Nós descontinuamos `React.render()` em favor de `ReactDOM.render()` no React 0.14, e removemos APIs específicas do DOM do `React` completamente no React 15. Contudo, a implementação do React DOM continua [secreta dentro do pacote do React](https://www.reddit.com/r/javascript/comments/3m6wyu/found_this_line_in_the_react_codebase_made_me/cvcyo4a/).

No React 15.4.0, nós estamos finalmente movendo a implementção do React DOM para o pacote React DOM. O pacote React irá ter apenas código relacionado a renderização como `React.Component` e `React.createElement()`.

Isto resolve algumas issues de longa data, como [erros](https://github.com/facebook/react/issues/7386) quando você importa o React DOM no mesmo arquivo de renderizar [testes de snapshot](https://facebook.github.io/jest/blog/2016/07/27/jest-14.html).

**Se você utiliza apenas as APIs oficiais e documentadas do React você não precisa mudar nada na sua aplicação**

No entanto, exste a possibildade de você ter importado APIs privadas do `react/lib/*`, ou que um pacote que você depende as utiliza. Nós gostaríamos de lembrar que isso nunca foi suportado e que suas aplicações não devem depender de APIs internas. O mecanismo interno do React irá continuar mudando enquanto trabalhamos para melhorar o React.

Outra coisa a se observar é que o servidor React DOM tem quase o mesmo tamanho que React DOM pois ele tem a sua própria cópia do reconciliador do React. Nós não recomendamos o uso do servidor React DOM no cliente na maioria dos casos.
